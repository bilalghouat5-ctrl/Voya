...
export default function ProfileTab({ user, onUpdateUser, onAddNotification, onSetHideBottomNav, onChangeTab, onShowLoginSignup, onToggleHostMode }: ProfileTabProps) {
  ...
  const isAnyModalOpen = showAccountModal || showGiftCardsModal || showWhyTuroModal || showBecomeHostModal || showLegalModal || showSupportModal || showTopUpSheet || showLicenseVerification || showAgencyModal || showLanguageModal || showLogoutConfirm;
  ...
  const [chatMessages, setChatMessages] = useState<Array<{ sender: "user" | "bot"; text: string }>>([
    { sender: "bot", text: "مرحباً! أنا مساعد درايف RENT الذكي بالجزائر 🤖. كيف يمكنني مساعدتك بخصوص كراء السيارات، شحن المحفظة، أو توثيق حسابك اليوم؟" }
  ]);
  ...
  const handleSendSupportMessage = () => {
    ...
    // Automated smart Arabic answers tailored for Algeria's درايف RENT users
    setTimeout(() => {
      let botReply = "شكراً لتواصلك مع دعم درايف RENT الجزائر. تم تحويل استفسارك إلى أحد ممثلي الخدمة في قسنطينة/الجزائر العاصمة وسيتم الرد هاتفياً خلال دقائق.";
  ...
  // Safe custom logout handler bypassing iframe restrictions
  const handleConfirmLogout = () => {
    ...
    onAddNotification({
      ...
      content: "تم تبديل الحساب وتسجيل الخروج بنجاح للعودة لوضع الزائر.",
      type: "system",
      date: "الآن",
      read: false
    });
  ...
  return (
    <div id="profile-tab-view" className="flex flex-col h-full bg-black text-white overflow-y-auto no-scrollbar pb-24 text-right">
      
      {user?.isGuest ? (
        /* GUEST PROFILE VIEW - Matching Turo App exactly as requested */
        <>
          {/* Profile Header Block matching the screenshot - Translated to Arabic RTL */}
          <div className="px-6 pt-11 pb-4 flex items-center gap-4 cursor-pointer" dir="rtl" onClick={onShowLoginSignup}>
            {/* Circular Avatar outline placeholder */}
            <div 
              className="w-[58px] h-[58px] rounded-full overflow-hidden bg-zinc-800 border border-zinc-700/40 flex items-center justify-center cursor-pointer select-none active:scale-95 transition-transform shrink-0"
            >
              <UserIcon className="w-8 h-8 text-zinc-400 stroke-[1.5]" />
            </div>
            
            {/* Name & Log In or Sign up Link */}
            <div className="text-right">
              <h1 className="text-[20px] font-black text-white tracking-tight leading-none mb-1.5">
                مرحباً بك
              </h1>
              <button 
                className="text-[13.5px] text-[#5c61ec] hover:text-[#7f83f2] font-semibold transition-colors leading-none block text-right font-sans focus:outline-none cursor-pointer"
              >
          ...
          {/* Become a Host Card designed exactly like the screenshot - Translated to Arabic RTL */}
          <div id="become-host-card" className="mx-6 my-4 bg-[#1a1a1c] rounded-[16px] overflow-hidden flex border border-[#262629]/60 shadow-lg relative min-h-[160px]" dir="rtl">
            {/* Right Side (Text content) */}
            <div className="flex-1 p-[20px] pl-4 flex flex-col justify-between text-right z-10">
              <div>
                <h2 className="text-[18px] font-black text-white leading-tight">
                  كن مستضيفاً
                </h2>
                <p className="text-[11.5px] text-zinc-400 font-medium mt-1.5 leading-relaxed max-w-[190px]">
                  انضم إلى آلاف المستضيفين الذين يبنون أعمالهم ويكسبون دخلاً حقيقياً على تورو (Turo).
                </p>
              </div>
              <div className="mt-4">
                <button 
                  onClick={onShowLoginSignup}
                  className="px-5 py-2.5 bg-[#5c61ec] hover:bg-[#4b50d3] active:scale-95 transition-all text-[11px] font-extrabold text-white rounded-[10px] shadow-[0_4px_14px_rgba(92,97,236,0.3)] cursor-pointer border border-[#6f73f7]/20 font-sans"
                >
                  معرفة المزيد
                </button>
              </div>
            ...
            <div className="w-[36%] shrink-0 relative bg-zinc-900">
              <img 
              ...
              />
              <div className="absolute inset-y-0 right-0 w-8 bg-gradient-to-l from-[#1a1a1c] to-transparent pointer-events-none" />
            </div>
          </div>

          {/* Divider */}
          <div className="border-b border-[#222224] mx-6 my-2.5" />

          {/* Menu Options List matching the screenshot format exactly - Translated to Arabic RTL */}
          <div className="flex flex-col" dir="rtl">
            {/* Why choose Turo */}
            <button 
              onClick={() => setShowWhyTuroModal(true)}
              className="w-full px-6 py-4 flex items-center gap-4 hover:bg-zinc-900/10 active:bg-zinc-900/20 transition-all text-right cursor-pointer"
            >
              <Compass className="w-[18px] h-[18px] text-zinc-100 stroke-[1.8]" />
              <span className="text-[15px] font-bold text-zinc-100">لماذا تختار تورو (Turo)</span>
            </button>
            ...
            <button 
              ...
              className="w-full px-6 py-4 flex items-center gap-4 hover:bg-zinc-900/10 active:bg-zinc-900/20 transition-all text-right cursor-pointer"
            >
              <HelpCircle className="w-[18px] h-[18px] text-zinc-100 stroke-[1.8]" />
              <span className="text-[15px] font-bold text-zinc-100">الحصول على المساعدة</span>
            </button>
            ...
            <button 
              ...
              className="w-full px-6 py-4 flex items-center gap-4 hover:bg-zinc-900/10 active:bg-zinc-900/20 transition-all text-right cursor-pointer"
            >
              <FileText className="w-[18px] h-[18px] text-zinc-100 stroke-[1.8]" />
              <span className="text-[15px] font-bold text-zinc-100">الشؤون القانونية</span>
            </button>

            {/* Version Code Label */}
            <div className="px-6 mt-5 mb-16 select-none text-right">
              <span className="text-[12px] font-semibold text-zinc-500 tracking-wide font-mono">
                الإصدار 26.22.0
              </span>
            </div>
          </div>
        ...
        /* STANDARD REGISTERED USER PROFILE VIEW */
        <>
            ...
            <div className="flex-1 p-[22px] pr-6 flex flex-col justify-between text-right z-10">
              <div>
                ...
                <p className="text-[11px] text-zinc-300 font-medium mt-1.5 leading-relaxed">
                  انضم إلى آلاف المضيفين الذين يبنون مشاريعهم ويحققون دخلاً معتبراً على درايف RENT.
                </p>
              </div>
              <div className="mt-4">
                <button 
          ...
          <div className="flex flex-col" dir="rtl">
            ...
            {onToggleHostMode && (
              <>
                <button 
                  ...
                  className="w-full px-6 py-4.5 flex items-center justify-between hover:bg-zinc-900/30 active:bg-zinc-900/50 transition-colors text-right cursor-pointer text-[#5c61ec]"
                >
                  <div className="flex items-center gap-4">
                    <TrendingUp className="w-5 h-5 text-[#5c61ec] stroke-[2]" />
                    <span className="text-sm font-bold text-[#5c61ec]">التحويل إلى وضع المضيف (Host Mode)</span>
                  </div>
                  <span className="text-[10px] bg-[#5c61ec]/15 text-[#5c61ec] px-2 py-0.5 rounded-full font-bold">نشط</span>
                </button>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm font-bold text-white">اسأل درايف RENT</span>
              </div>
              {/* Sparkly purple Chip */}
            ...
            >
              ...
              <span className="text-sm font-bold text-white">بطاقات الهدايا (درايف RENT Gift)</span>
            </button>

            {/* Why choose درايف RENT */}
            <button 
              onClick={() => setShowWhyTuroModal(true)}
              className="w-full px-6 py-4.5 flex items-center gap-4 hover:bg-zinc-900/30 active:bg-zinc-900/50 transition-colors text-right cursor-pointer"
            >
              <Compass className="w-5 h-5 text-white stroke-[2]" />
              <span className="text-sm font-bold text-white">لماذا تختار درايف RENT</span>
            </button>

            {/* Get help */}
            <button 
              onClick={() => setShowSupportModal(true)}
              className="w-full px-6 py-4.5 flex items-center gap-4 hover:bg-[#19191b]/50 transition-colors text-right cursor-pointer"
            >
            ...
            <button 
              ...
              className="w-full px-6 py-4.5 flex items-center gap-4 hover:bg-zinc-900/30 active:bg-zinc-900/50 transition-colors cursor-pointer"
            >
              <Globe className="w-5 h-5 text-[#5c61ec] stroke-[2]" />
              <div className={`flex flex-col ${isRtl ? "text-right" : "text-left"}`}>
                <span className="text-sm font-bold text-white">{t("تغيير لغة التطبيق")}</span>
                <span className="text-[10px] text-indigo-400 font-extrabold uppercase">
                  {language === "ar" ? "العربية" : language === "en" ? "English" : language === "fr" ? "Français" : "ⵜⴰⵎⴰⵣⵉⵖⵜ (Amazigh)"}
                </span>
              </div>
            </button>
            ...
            <button 
              ...
              className="w-full px-6 py-4.5 text-right font-bold text-sm text-red-500 hover:bg-zinc-900/20 active:bg-zinc-900/40 cursor-pointer"
            >
              تسجيل الخروج
            </button>
      ...
      <AnimatePresence>
        {showAccountModal && (
          <div className="fixed inset-0 z-50 flex flex-col bg-black text-white text-right font-sans overflow-hidden" dir="rtl">
            <motion.div
              ...
              {accountSubView === "menu" ? (
                <div className="w-full h-full flex flex-col bg-black overflow-y-auto no-scrollbar pb-[108px]" dir="rtl">
                  ...
                  <div className="relative flex items-center justify-center w-full h-16 shrink-0 border-b border-zinc-900/10">
                    <button
                      ...
                      className="absolute right-6 w-10 h-10 rounded-full glass-back-btn flex items-center justify-center text-white cursor-pointer"
                      title="خروج"
                    >
                ...
                <div className="w-full h-full flex flex-col bg-black overflow-y-auto no-scrollbar pb-[108px]" dir="rtl">
                  ...
                  <div className="flex-1 px-6 pt-6 divide-y divide-zinc-900/60 pb-12">
                    ...
                    <div 
                      onClick={() => {
                        alert("تم تأمين الجلسة بكلمة مرور نشطة. لإعادة التعيين يرجى استخدام رابط استعادة البريد الإلكتروني.");
                      }}
                      className="py-5 text-right cursor-pointer hover:bg-zinc-900/10 transition-colors"
                    >
                      <div className="flex justify-between items-center">
                ...
                <div className="w-full h-full flex flex-col bg-black overflow-y-auto no-scrollbar pb-[108px]" dir="rtl">
                  ...
                  <div className="flex-1 px-6 pt-5 divide-y divide-zinc-900/40 pb-12 text-right">
                    ...
                    <div className="py-6">
                      ...
                      {/* Row 3: Promos and announcements */}
                      <div className="flex justify-between items-center py-4">
                        <span className="text-[15px] font-bold text-white">العروض والإعلانات</span>
                        <button
                          onClick={() => {
                            ...
                            onAddNotification({
                              ...
                              title: "تحديث البريد ✉️",
                              content: `تم ${nextState ? "تفعيل" : "إيقاف"} تلقي عروض الترويج لـ درايف RENT DZ عبر البريد الإلكتروني.`,
                              type: "system",
                              date: "الآن",
                              read: false
                            });
                  ...
                  <div className="flex-1 overflow-y-auto no-scrollbar px-6 pb-24 space-y-6">
                    ...
                    >
                      <div className="border-t border-[#1c1c1e] py-4.5 flex justify-between items-center text-right hover:text-zinc-300 transition-colors">
                        ...
                        <span className="text-[14px] font-bold text-zinc-100 group-hover:text-white">تغيير الصورة الشخصية</span>
                      </div>
      ...
      <AnimatePresence>
        {showSupportModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-4 text-white text-right font-sans overflow-hidden" dir="rtl">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-zinc-950 rounded-[28px] w-full max-w-sm h-[75%] flex flex-col overflow-hidden border border-zinc-850"
            >
              <div className="bg-zinc-900 p-4 border-b border-zinc-800/80 flex justify-between items-center shrink-0">
                <button
                  onClick={() => setShowSupportModal(false)}
                  className="w-10 h-10 rounded-full bg-white/[0.08] backdrop-blur-[12px] border border-white/20 hover:bg-white/[0.18] hover:border-white/35 active:scale-95 transition-all duration-300 flex items-center justify-center text-white cursor-pointer shadow-md"
                  title="إغلاق التقرير"
                >
                  <X className="w-5 h-5" />
                </button>
                <div className="flex items-center gap-2">
                  <div className="text-right">
                    <h3 className="text-xs font-extrabold text-white">مساعد درايف RENT الذكي 🤖</h3>
                    <p className="text-[9px] text-[#5c61ec] font-bold">دعم فني • الجزائر بالكامل</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-3 bg-zinc-900 border-t border-zinc-800/90 flex gap-2 shrink-0">
                <input 
                  ...
                  className="flex-1 text-xs bg-zinc-950 border border-zinc-800 rounded-full px-4 py-2.5 outline-none focus:border-[#5c61ec] text-right"
                />
                <button
      ...
      <AnimatePresence>
        {showGiftCardsModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-md p-4 text-white text-right mb-0" dir="rtl">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
            ...
            >
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-extrabold text-white">بطاقات الهدايا (درايف RENT Gift)</h3>
                </div>
              </div>

              <div className="space-y-4 text-right text-[11px] leading-relaxed text-zinc-400">
                <div className="bg-gradient-to-br from-purple-900/40 via-purple-950/20 to-zinc-900 p-4 rounded-xl border border-purple-500/20">
                  <p className="font-extrabold text-white text-xs mb-1">🎁 اشحن رصيد حسابك أو أهديه لمن تحب</p>
                  <p className="text-[10px] text-zinc-300">متاحة بقيمة: <span className="text-purple-400 font-bold">5,000 دج</span> • <span className="text-purple-400 font-bold">10,000 دج</span> • <span className="text-purple-400 font-bold">25,000 دج</span>.</p>
                </div>
                
                <div className="space-y-2">
                  <p>• للاستعمال, أدخل رمز بطاقة الهدايا الخاص بك أدناه لشحن رصيد محفظتك الرقمية فوراً:</p>
                  <input
                    type="text"
                    placeholder="مثال: VOYA-GIFT-5000"
                    className="w-full text-xs font-mono bg-zinc-900 border border-zinc-800 rounded-xl p-3 text-purple-400 focus:border-[#5c61ec] outline-none text-center uppercase"
                  />
                  <button
                    onClick={() => {
                      onAddNotification({
                        id: "gift_redeem_" + Date.now(),
                        title: "تم استلاف رمز بطاقة الهدايا 🎟️",
                        content: "سيتم مراجعة رمز بطاقة الهدايا وإضافة الرصيد لمحفظتك خلال دقائق بعد التحقق من صحته.",
                        type: "system",
                        date: "الآن",
                        read: false
                      });
                    ...
                    className="w-full py-2.5 bg-[#5c61ec] hover:bg-[#4b50d3] text-white rounded-xl text-xs font-black shadow-lg"
                  >
                    تفعيل بطاقة الهدايا
                  </button>
                </div>
            ...
            >
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-extrabold text-white">لماذا تختار درايف RENT؟</h3>
                </div>
              </div>
              ...
              <div className="space-y-4 text-right text-[10.5px] leading-relaxed text-zinc-400">
                ...
                <div className="space-y-1">
                  <h4 className="font-extrabold text-white">🚗 مساعدة على الطريق 24/7</h4>
                  <p>دعم فني وتغطية تأمينية كاملة ضد الحوادث والأعطال لضمان راحتك وضمان سلامتك على مدار الساعة.</p>
                </div>
              </div>
            </motion.div>
          </div>
            ...
            >
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-extrabold text-white">كيف تصبح مستضيفاً على درايف RENT؟</h3>
                </div>
              </div>
              ...
              <div className="space-y-4 text-right text-[10.5px] leading-relaxed text-zinc-300">
                ...
                <div className="space-y-1 bg-zinc-900/30 p-3 rounded-[15px] border border-zinc-900">
                  ...
                  <p className="text-zinc-400 text-[10px]">احصل على مدفوعاتك الكترونياً في محفظة درايف RENT، وحوّلها لحسابك مباشرة مع تغطية تأمينية كاملة لعقودك.</p>
                </div>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-md p-4 text-white text-right" dir="rtl">
            ...
            >
              ...
              <div className="space-y-3 text-right text-[10.5px] leading-relaxed text-zinc-400 max-h-[50vh] overflow-y-auto no-scrollbar">
                <p className="font-black text-white text-xs">1. الأهلية ورخص القيادة</p>
                <p>يجب على كافة مستخدمي تطبيق Voya الجزائر أن يكونوا قد تجاوزوا سن 21 وأكثر، مع رخصة سياقة وطنية سارية المفعول لأكثر من سنتين كاملتين.</p>
                
                <p className="font-black text-white text-xs">2. العقود والمسؤوليات</p>
                ...
                <p className="font-black text-white text-xs">3. الكفالة ودفع الرصيد</p>
      ...
      <AnimatePresence>
        {showLanguageModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-md p-4 text-white" dir={dir}>
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
            ...
            >
              <div className="flex justify-between items-center flex-row">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-extrabold text-white">{t("اختر لغة التطبيق")}</h3>
                </div>
              </div>

              <div className="space-y-2.5 font-sans">
                {[
                  { code: "ar", label: "العربية", flag: "🇩🇿", desc: "Arabe / Arabic" },
                  { code: "en", label: "English", flag: "🇬🇧", desc: "الإنجليزيّة / Anglais" },
                  { code: "fr", label: "Français", flag: "🇫🇷", desc: "الفرنسية / French" },
                  { code: "amazigh", label: "ⵜⴰⵎⴰⵣⵉⵖⵜ", flag: "ⵣ", desc: "Amazigh / الأمازيغية" }
                ].map((item) => {
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        onAddNotification({
                          ...
                          title: item.code === "ar" ? "تم تغيير لغة التطبيق 🌐" : item.code === "en" ? "App language updated! 🌐" : item.code === "fr" ? "Langue de l'app mise à jour ! 🌐" : "Tasegda n tutlayt tyules! 🌐",
                          content: item.code === "ar" ? "تم تحويل لغة واجهة تطبيق درايف RENT بالجزائر للعربية بنجاح." : item.code === "en" ? "Drive RENT app interface successfully changed to English." : item.code === "fr" ? "L'interface de Drive RENT Algérie a été configurée en français." : "Tutlayt n Drive RENT di Lzayer tura s Tamaziɣt.",
                          type: "system",
                          date: "الآن",
                          read: false
                        });
                      ...
                      className={`w-full p-4 rounded-[16px] border flex items-center justify-between transition-all cursor-pointer text-left ${
                        isSelected 
                          ? "bg-[#5c61ec]/10 border-[#5c61ec] shadow-[0_0_12px_rgba(92,97,236,0.15)]" 
                          : "bg-zinc-900/45 border-zinc-800 hover:bg-zinc-900/70"
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-lg">{item.flag}</span>
                        <div className="flex flex-col">
                          <span className={`text-xs font-bold ${isSelected ? "text-white" : "text-zinc-200"}`}>{item.label}</span>
                          <span className="text-[9px] text-zinc-500 font-medium">{item.desc}</span>
                        </div>
                      </div>
                      {isSelected && (
                        <div className="w-5 h-5 rounded-full bg-[#5c61ec] flex items-center justify-center">
      ...
      <AnimatePresence>
        {showTopUpSheet && (
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/85 backdrop-blur-sm text-white text-right" dir="rtl">
            <motion.div
              initial={{ y: "100%" }}
              animate={{ y: 0 }}
              exit={{ y: "100%" }}
            ...
            >
              ...
              {isTopUpSuccess ? (
                ...
                <form onSubmit={handleWalletTopUp} className="space-y-4">
                  ...
                  <div>
                    <label htmlFor="card-number" className="text-[9px] font-bold text-zinc-500 block mb-1">رقم بطاقة الذهبية لبريد الجزائر (16 رقم)*</label>
                    <input 
                      id="card-number"
                      type="text"
                      required
      ...
      <AnimatePresence>
        {showLicenseVerification && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm p-4 text-white text-right overflow-hidden" dir="rtl">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-zinc-950 rounded-3xl p-5 w-full max-w-sm border border-zinc-800 max-h-[88%] overflow-y-auto no-scrollbar"
            >
      ...
      {/* Sleek Custom Inside-App Logout Confirmation Modal */}
      <AnimatePresence>
        {showLogoutConfirm && (
          <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/60 backdrop-blur-[2px]" dir="rtl">
            <motion.div
              initial={{ scale: 0.9, opacity: 0, y: 15 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.9, opacity: 0, y: 15 }}
              transition={{ type: "spring", damping: 26, stiffness: 360 }}
              className="bg-[#1c1c1e] border border-white/5 rounded-[48px] p-2.5 flex items-center gap-2.5 shadow-[0_24px_50px_-12px_rgba(0,0,0,0.85)] w-full max-w-[330px]"
            >
              {/* Right Button in RTL: إلغاء (Cancel with white text) */}
              <button
                onClick={() => setShowLogoutConfirm(false)}
                className="flex-1 py-4.5 px-6 bg-[#2c2c2e] hover:bg-[#343438] active:scale-[0.97] transition-all font-sans font-bold text-sm text-zinc-100 rounded-full text-center cursor-pointer select-none"
              >
                إلغاء
              </button>

              {/* Left Button in RTL: تسجيل الخروج (Log out with red text) */}
              <button
                onClick={handleConfirmLogout}
                className="flex-1 py-4.5 px-6 bg-[#2c2c2e] hover:bg-[#343438] active:scale-[0.97] transition-all font-sans font-bold text-sm text-[#ff4b4b] rounded-full text-center cursor-pointer select-none"
              >
                تسجيل الخروج
              </button>
            </motion.div>
