      ...
      <AnimatePresence mode="wait">
        {!activeChatId && activeTab === "renters" ? (
              ...
              {renterChats.map((chat) => (
                ...
                >
                  <div className="relative shrink-0 select-none">
                    <img src={chat.avatar} className="w-12 h-12 rounded-full object-cover border border-zinc-950" referrerPolicy="no-referrer" />
                    {chat.unread && (
                      <span className="absolute top-0 right-0 w-3 h-3 bg-[#5c61ec] rounded-full border-2 border-black" />
                    )}
                  </div>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4 no-scrollbar flex flex-col justify-end bg-black" dir="rtl">
                    ...
                    <div key={i} className={`flex w-full ${isHost ? 'justify-start' : 'justify-end'}`}>
                      <div className={`max-w-[80%] rounded-2xl p-3.5 px-4 shadow-md relative ${
                        isHost 
                          ...
                          : 'bg-zinc-900 text-zinc-300 border border-zinc-950 rounded-tl-none text-right font-sans font-medium'
                      }`}>
                    ...
                    <div key={i} className={`flex w-full ${isHost ? 'justify-start' : 'justify-end'}`}>
                      <div className={`max-w-[80%] rounded-2xl p-3.5 px-4 shadow-md relative ${
                        isHost 
                          ...
                          : 'bg-zinc-900 text-zinc-300 border border-zinc-950 rounded-tl-none text-right'
                      }`}>
              ...
              {isTyping && (
                <div className="flex w-full justify-end">
                  <div className="bg-zinc-900 border border-zinc-950 p-3 px-4 rounded-2xl rounded-tl-none">
            ...
            <div className="p-4 bg-[#111113] border-t border-zinc-900 shrink-0">
              <form onSubmit={handleSendMessage} className="flex gap-2.5 items-center rounded-2xl bg-zinc-950/80 border border-zinc-900 p-1.5 pr-4">
                <input 
                  type="text"
                  value={typedMessage}
