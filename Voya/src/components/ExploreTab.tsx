    ...
    <div id="explore-tab-view" className={`flex flex-col h-full bg-black text-white ${(selectedCar || selectedAgency || isSearchOverlayOpen) ? 'overflow-hidden' : 'overflow-y-auto no-scrollbar'} pb-24 select-none relative`}>
      ...
      <AnimatePresence>
        {showFeaturesModal && selectedCar && (
          <div id="features-sheet-overlay" className="absolute inset-0 z-[170] flex items-center justify-center bg-black overflow-hidden">
            <motion.div
              initial={{ x: "-100%" }}
              animate={{ x: 0 }}
              exit={{ x: "-100%" }}
            ...
            >
              ...
              <div className="flex items-center px-4 py-4 bg-black border-b border-zinc-900/60 shrink-0 select-none relative" dir="rtl">
                <button
                  ...
                  className="absolute right-4 p-2 rounded-full hover:bg-zinc-900 text-white flex items-center justify-center transition-colors cursor-pointer active:scale-95"
                  aria-label="إغلاق"
                >
      ...
      <AnimatePresence>
        {showReviewsModal && selectedCar && (() => {
          ...
          return (
            <div id="reviews-sheet-overlay" className="absolute inset-0 z-[170] flex items-center justify-center bg-black overflow-hidden">
              <motion.div
                initial={{ x: "-100%" }}
                animate={{ x: 0 }}
                exit={{ x: "-100%" }}
      ...
      <AnimatePresence>
        {selectedAgency && (
          <div id="agency-sheet-overlay" className="absolute inset-0 z-[170] flex items-center justify-center bg-black overflow-hidden">
            <motion.div
              initial={{ x: "-100%" }}
              animate={{ x: 0 }}
              exit={{ x: "-100%" }}
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6 no-scrollbar">
                ...
                <div className="flex flex-col items-center justify-center text-center space-y-3 pt-4 select-none">
                  <div className="w-[84px] h-[84px] rounded-full bg-zinc-900 border-2 border-zinc-800/80 overflow-hidden flex items-center justify-center relative">
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-1.5 justify-center">
                      <h2 className="text-[22px] font-black tracking-tight text-white font-sans">{selectedAgency.name}</h2>
                      <svg viewBox="0 0 24 24" fill="currentColor" className="w-[18px] h-[18px] text-[#5c61ec] shrink-0">
                        <path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10 10-4.5 10-10S17.5 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z" />
                      </svg>
                    </div>
                    <p className="text-[12.5px] text-zinc-400 font-sans flex items-center gap-1 justify-center">
                      <span>حساب مستضيف ذهبي معتمد</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="px-6 text-right space-y-1">
                  <div className="flex justify-between items-center pb-2">
                    <span className="text-[10px] font-black bg-purple-950/45 border border-purple-900/45 text-purple-400 px-3 py-0.5 rounded-full font-sans">
                      {cars.filter(c => c.agencyId === selectedAgency.id || c.agencyName === selectedAgency.name || c.hostName === selectedAgency.name).length} سيارات
                    </span>
                    <h3 className="text-xl font-bold text-white tracking-tight font-sans">
                      أسطول السيارات المتوفرة
                    </h3>
                  </div>
                </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="pt-3 pb-2 text-right space-y-1.5 flex flex-col justify-between">
                            ...
                            {/* Footer block with Save badge and price */}
                            <div className="flex justify-between items-end pt-3">
                              {/* Save green tag */}
                              <div className="bg-emerald-950/40 border border-emerald-900/45 text-emerald-400 text-[11px] font-bold px-3 py-1 rounded-lg flex items-center gap-1 select-none font-sans">
                                ...
                                <span>وفر {savings.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ".")} دج</span>
                              </div>

                              {/* Dynamic price logic */}
                              <div className="text-right">
                                <div className="flex items-baseline gap-1.5 justify-end font-sans">
                                  <span className="text-zinc-500 line-through font-bold text-xs">{originalPrice.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ".")} دج</span>
                                  <span className="text-white font-extrabold text-base font-sans">{carData.pricePerDay.toLocaleString()} دج</span>
                                  <span className="text-[12px] text-white font-bold">المجموع</span>
                                </div>
                                <span className="text-[10px] text-zinc-500 font-bold block pt-0.5 font-sans">قبل الضرائب والرسوم</span>
                              </div>
                            </div>
                ...
                <div className="px-6 pt-2 space-y-6 text-right pb-10">
                  ...
                  <button className="w-full py-3.5 border border-zinc-850 hover:bg-zinc-900 text-zinc-300 rounded-[14px] text-xs font-extrabold transition-all active:scale-[0.98] cursor-pointer mt-4 font-sans">
                    عرض جميع التقييمات
                  </button>
                </div>
      ...
      <AnimatePresence>
        {isSearchOverlayOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[180] bg-[#0d0d10] flex flex-col font-sans select-none overflow-hidden"
            dir="rtl"
          >
