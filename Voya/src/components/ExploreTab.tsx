...
export default function ExploreTab({ cars, user, onUpdateUser, onAddBooking, onAddNotification, onSetHideBottomNav, onShowLoginSignup }: ExploreTabProps) {
  ...
  return (
    <div id="explore-tab-view" className={`flex flex-col h-full bg-black text-white ${(selectedCar || selectedAgency || isSearchOverlayOpen) ? 'overflow-hidden' : 'overflow-y-auto no-scrollbar'} pb-24 select-none relative`}>
      ...
      <div id="explore-hero-header" className="px-4 pt-3.5 pb-3 shrink-0 bg-black/95 backdrop-blur-md sticky top-0 z-20">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-2">
            ...
            {/* Premium Filter Drawer Toggle Button */}
            <button
              onClick={() => setShowFilters(!showFilters)}
              className={`w-[52px] h-[52px] rounded-[18px] flex items-center justify-center border transition-all duration-300 shrink-0 ${
                showFilters 
                  ? "bg-[#5c61ec] border-[#5c61ec] text-white shadow-md active:scale-95 shadow-[#5c61ec]/20" 
                  : "bg-[#121214] border-zinc-900 text-zinc-300 hover:bg-[#1a1a1f] hover:border-zinc-850"
              }`}
              title="تصفية وفلترة السيارات"
            >
        ...
        {isFilteringActive ? (
          ...
          <div className="space-y-7">
            ...
            {(currentPill === "الكل" || currentPill === "luxury") && luxuryCars.length > 0 && (
              <div className="space-y-3.5 px-1 animate-fade-in duration-300">
                <div className="text-right font-sans" dir="rtl">
            ...
            {currentPill === "Airports" && (
              <div className="space-y-4 animate-fade-in duration-300" dir="rtl">
      ...
      <AnimatePresence>
        {showBookingSheet && selectedCar && (
          <div id="booking-sheet-overlay" className="absolute inset-0 z-[110] flex items-end justify-center bg-black/85 backdrop-blur-xs">
            <motion.div
              initial={{ y: "100%" }}
              animate={{ y: 0 }}
              exit={{ y: "100%" }}
            ...
            >
              ...
              {bookingSuccess ? (
                ...
                <div className="space-y-4">
                  ...
                  {/* Select Dates - Premium Summary Button */}
                  <div 
                    onClick={() => setShowCustomDatePicker(true)}
                    className="p-3.5 bg-zinc-900 hover:bg-zinc-850 border border-zinc-800/50 rounded-xl flex items-center justify-between text-right cursor-pointer group active:scale-98 transition-all"
                  >
                    <div className="space-y-1">
                      <span className="text-[10px] text-zinc-500 font-extrabold uppercase">تواريخ الاستلام والإرجاع المعتمدة</span>
                      <div className="text-xs font-bold text-zinc-200 font-sans space-y-0.5">
                        <p>{formatTripDate(startDate, pickupTime)}</p>
                        <p>{formatTripDate(endDate, returnTime)}</p>
                      </div>
                    </div>
                  ...
                  <div className="bg-zinc-905 p-3 rounded-2xl border border-zinc-900 flex flex-col gap-2 font-semibold">
                    ...
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-zinc-500">تأمين درايف RENT والوساطة (5%):</span>
                  ...
                  <div className="flex justify-between items-center pt-2">
                    ...
                    <button
                      ...
                      className="px-5 py-2.5 bg-[#5c61ec] hover:bg-[#4b50d3] text-white rounded-full text-xs font-black flex items-center gap-1.5 shadow-lg border border-[#6f73f7]/25 active:scale-95 transition-all text-center cursor-pointer"
                    >
          ...
          return (
            <div id="custom-datepicker-overlay" className="absolute inset-0 z-[130] bg-black overflow-hidden flex flex-col">
              ...
              >
                ...
                <div className="h-14 bg-black border-b border-zinc-900 flex items-center px-4 justify-between shrink-0">
                  <button
                    onClick={() => setShowCustomDatePicker(false)}
                    className="w-10 h-10 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-400 hover:text-white hover:bg-zinc-850 active:scale-95 transition-all cursor-pointer"
                    title="الرجوع"
                  >
                    <X className="w-5 h-5 text-white" />
                  </button>
                  <span className="text-[17px] font-bold text-white tracking-wide font-sans">Trip Dates</span>
                  <div className="w-10 h-10" /> {/* Spacer for alignment */}
                </div>

                {/* Display dates selection on top (Mon, 17 Aug etc.) */}
                <div className="bg-[#040405] py-4 px-6 border-b border-zinc-900 select-none shrink-0" dir="ltr">
                  <div className="flex items-center justify-between">
                    {/* Left/Pickup date */}
                    <div className="flex-1 text-center">
                      <div className="text-[20px] font-bold text-white tracking-tight font-sans">
                        {tempStartDate ? formatTripDateEN(tempStartDate) : "Select Date"}
                      </div>
                      <div className="text-[13px] text-zinc-400 mt-1 font-sans font-medium">
                        {tempPickupTime}
                      </div>
                    </div>

                    {/* Styled centered white arrow pill */}
                    <div className="mx-4 flex items-center justify-center">
                      <div className="w-6 h-[2px] bg-white relative">
                        <div className="absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/3 w-1.5 h-1.5 border-t border-r border-white rotate-45" />
                      </div>
                    </div>
                    ...
                    <div className="flex-1 text-center">
                      <div className="text-[20px] font-bold text-white tracking-tight font-sans">
                        {tempEndDate ? formatTripDateEN(tempEndDate) : "Select Date"}
                      </div>
                      <div className="text-[13px] text-zinc-405 mt-1 font-sans font-medium">
                        {tempReturnTime}
                      </div>
                    </div>
                  </div>
                ...
                {/* Weekdays indicator headers M T W T F S S formatted precisely (starts on Monday) */}
                <div className="grid grid-cols-7 text-center py-2.5 bg-[#08080a] text-[11px] text-zinc-500 font-extrabold tracking-widest border-b border-zinc-900 shrink-0 select-none">
                  <span>M</span>
                  <span>T</span>
                  <span>W</span>
                  <span>T</span>
                ...
                <div className="flex-grow overflow-y-auto no-scrollbar p-4 space-y-8 bg-black pb-12">
                  ...
                  {/* JUNE 2026 */}
                  <div>
                    <h4 className="text-center text-[13.5px] font-bold text-zinc-400 font-sans tracking-wide mb-4">JUNE 2026</h4>
                        ...
                        return (
                          ...
                          >
                            ...
                            {isSelStart && tempEndDate && (
                              <div className="absolute inset-y-1 right-0 left-1/2 bg-[#5c61ec]/15 rounded-r-none" />
                            )}
                            {isSelEnd && tempStartDate && (
                              <div className="absolute inset-y-1 left-0 right-1/2 bg-[#5c61ec]/15 rounded-l-none" />
                            )}
                  ...
                  {/* JULY 2026 */}
                  <div>
                    <h4 className="text-center text-[13.5px] font-bold text-zinc-400 font-sans tracking-wide mb-4">JULY 2026</h4>
                    <div className="grid grid-cols-7 gap-y-2 text-center">
                  ...
                  <div className="pt-2 select-none" dir="rtl">
                    <h5 className="text-[11px] tracking-widest font-extrabold text-zinc-500 mb-2.5 uppercase text-right px-1 font-sans">PICKUP</h5>
                  ...
                  <div className="pt-2 select-none" dir="rtl">
                    <h5 className="text-[11px] tracking-widest font-extrabold text-zinc-500 mb-2.5 uppercase text-right px-1 font-sans">RETURN</h5>
                    <div className="flex gap-2 overflow-x-auto no-scrollbar py-1 px-1">
                      {timeOptions.map((time) => {
                ...
                <div className="bg-black border-t border-zinc-900 px-6 py-4 flex items-center justify-between shrink-0" dir="ltr">
                  <button
