      ...
      <AnimatePresence mode="wait">
        {!activeId ? (
          ...
          >
                    ...
                    >
                      ...
                      {tab.count > 0 && (
                        <span className={`text-[9px] font-sans font-black rounded-full px-1.5 py-0.5 min-w-[16px] text-center leading-none ${
                          isActive ? "bg-white text-[#5c61ec]" : "bg-[#5c61ec]/15 text-[#5c61ec]"
                        }`}>
                          {tab.count}
                        </span>
                      )}
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar p-5 pb-24 bg-black space-y-4">
              {currentList.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-center bg-zinc-900 border border-zinc-850 rounded-[24px] p-6">
                  ...
                  {currentList.map((trip) => {
                    ...
                    return (
                      <div
                        ...
                        className="bg-zinc-900 border border-zinc-850 hover:border-zinc-700 hover:shadow-[#5c61ec]/5 p-4 rounded-[22px] flex flex-col gap-3.5 text-right cursor-pointer shadow-lg hover:shadow-2xl transition-all duration-300 relative group"
                      >
                        <div className="flex gap-4 flex-row">
                          <div className="relative shrink-0 w-[100px] h-[70px] bg-zinc-950 overflow-hidden rounded-xl border border-white/[0.05]">
                        ...
                        <div className="flex justify-between items-center border-t border-white/[0.03] pt-3.5 mt-0.5">
          ...
          >
            ...
            <div className="flex-grow p-4.5 overflow-y-auto no-scrollbar pb-24 bg-[#030303] space-y-4 text-right">
              
              {/* Car Info Card */}
              <div className="bg-zinc-900 border border-zinc-850 p-4 rounded-[22px] flex gap-4 text-right shadow-lg">
                <div className="relative shrink-0 w-24 h-16 bg-zinc-950 overflow-hidden rounded-xl border border-white/[0.05]">
              ...
              <div className="bg-zinc-900 border border-zinc-850 p-4 rounded-[22px] space-y-3.5 text-right shadow-lg relative overflow-hidden">
                <h3 className="text-xs font-black text-white border-b border-white/[0.04] pb-2.5 flex items-center gap-1.5">
                ...
                <div className="flex gap-2.5 pt-2.5 border-t border-white/[0.03]">
                  <button 
                    onClick={() => alert(`جاري الاتصال بهاتف المستأجر: ${selectedTrip?.renterPhone || "0554-12-42-12"}`)}
                    className="flex-grow cursor-pointer bg-zinc-950 hover:bg-white/[0.02] border border-white/[0.05] hover:border-white/[0.1] rounded-xl py-2 px-3 flex items-center justify-center gap-1.5 text-[11px] font-black text-zinc-300 transition-all active:scale-95"
                  >
              ...
              <div className="bg-zinc-900 border border-zinc-850 p-4 rounded-[22px] space-y-3 text-right shadow-lg font-sans">
                <h3 className="text-xs font-black text-white border-b border-white/[0.04] pb-2.5 flex items-center gap-1.5">
                ...
                <div className="space-y-2.5 pt-0.5">
                  ...
                  <div className="flex justify-between items-center text-right border-t border-white/[0.03] pt-2.5">
              ...
              <div className="bg-zinc-900 border border-zinc-850 p-4 rounded-[22px] space-y-3.5 text-right shadow-lg">
                <div className="flex justify-between items-baseline border-b border-white/[0.04] pb-2.5">
                  ...
                  <button 
                    ...
                    className="text-[10px] bg-white/[0.02] border border-white/[0.05] hover:border-[#5c61ec]/40 px-2 py-1 rounded-lg text-[#5c61ec] font-black cursor-pointer"
                  >
                    + إضافة صورة
                  </button>
                </div>
                ...
                <div className="flex gap-2.5 overflow-x-auto no-scrollbar py-1">
                  {preTripPhotos.map((photo, i) => (
                    <div key={i} className="relative w-20 h-16 rounded-xl overflow-hidden shrink-0 border border-white/[0.05] bg-zinc-950">
                      <img src={photo} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    </div>
                  ))}
                  <button 
                    onClick={() => uploadInspectionPhoto("pre")}
                    className="w-20 h-16 border-2 border-zinc-800 border-dashed rounded-xl flex flex-col items-center justify-center text-zinc-550 shrink-0 hover:text-white hover:border-[#5c61ec]/40 bg-zinc-950/40 transition-colors cursor-pointer"
                  >
              ...
              <div className="bg-zinc-900 border border-zinc-850 p-4 rounded-[22px] space-y-3.5 text-right shadow-lg">
                ...
                <div className="flex justify-start pt-1">
                  <button 
                    onClick={() => uploadInspectionPhoto("post")}
                    className="px-3.5 py-2.5 bg-zinc-950 border border-white/[0.05] hover:border-white/[0.1] rounded-xl text-[10.5px] font-black text-zinc-350 flex items-center gap-1.5 cursor-pointer active:scale-95 transition-all"
                  >
                    <Camera className="w-4 h-4 text-[#5c61ec]" />
              ...
              <div className="bg-zinc-900 border border-zinc-850 p-4 rounded-[22px] space-y-3.5 text-right shadow-lg">
                ...
                <div className="space-y-3.5 pt-1.5 font-sans">
                  <input
                    ...
                    className="w-full bg-zinc-950 border border-white/[0.05] rounded-xl p-3 text-xs text-white outline-none focus:border-amber-500 text-right leading-none placeholder-zinc-600 focus:bg-black transition-all"
              ...
              {selectedTrip?.status === "pending" && (
                <div className="flex gap-3 pt-3">
                  ...
                  <button 
                    ...
                    className="py-3.5 px-4 bg-[#141416] hover:bg-zinc-900 border border-white/[0.04] rounded-xl flex items-center justify-center text-red-500 font-bold text-xs active:scale-95 transition-all cursor-pointer"
                  >
                    <X className="w-4 h-4" />
                    <span>رفض الطلب</span>
                  </button>
