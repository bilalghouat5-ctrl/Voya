...
export default function HostTab({ cars, user, onAddCar, onDeleteCar, onToggleAvailability, onAddNotification, onSetHideBottomNav, onShowLoginSignup }: HostTabProps) {
  ...
  if (user?.isGuest) {
    return (
      <div className="w-full h-full bg-black text-white relative flex flex-col items-center justify-center p-6 text-center" dir="rtl">
        <div className="w-20 h-20 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center mb-5 relative">
          <MessageSquare className="w-9 h-9 text-zinc-500 stroke-[1.5]" />
          <div className="absolute top-1 right-1 w-3 h-3 rounded-full bg-[#5c61ec]" />
        </div>
        ...
        <p className="text-xs text-zinc-400 font-medium leading-relaxed max-w-xs mt-3">
          قم بتسجيل الدخول أو إنشاء حساب لعرض رسائل المستضيفين، التواصل مع دعم العملاء، أو إضافة سيارتك الخاصة وبدء الاستضافة لكسب ريع إضافي.
        </p>

        <button
          onClick={onShowLoginSignup}
          className="mt-8 px-8 py-3.5 bg-[#5c61ec] hover:bg-[#4b50d3] active:scale-95 text-white rounded-[14px] text-xs font-black tracking-wide transition-all duration-200 select-none shadow-[0_4px_16px_rgba(92,97,236,0.35)] cursor-pointer border border-[#6f73f7]/20"
        >
  ...
  return (
    <div className={`w-full h-full bg-black text-white relative flex flex-col overflow-hidden text-right ${activeChat ? "pb-0" : "pb-24"}`} dir="rtl">
          ...
          {currentInboxTab === "messages" ? (
            activeChat ? (
              ...
              <div className="flex flex-col h-full w-full bg-zinc-950 font-sans">
                {/* Chat header area */}
                <div className="bg-[#111113] px-3.5 py-3 border-b border-zinc-950 flex items-center justify-between shrink-0">
                  <button 
                ...
                {/* Messages Feed area scrollable list */}
                <div className="flex-1 overflow-y-auto no-scrollbar p-4 space-y-4 flex flex-col">
                    ...
                    return (
                      ...
                      >
                        <div className={`max-w-[78%] rounded-2xl p-3 px-4 shadow-sm relative ${
                          isUser 
                            ? 'bg-[#5c61ec] text-white rounded-br-none text-right font-sans font-bold shadow-lg shadow-[#5c61ec]/10' 
                            : 'bg-zinc-900 text-zinc-150 rounded-bl-none text-right border border-zinc-950'
                        }`}>
                  ...
                  {isTyping && (
                    <div className="flex w-full justify-end">
                      <div className="bg-zinc-900 text-zinc-150 rounded-2xl p-3 px-4 rounded-bl-none border border-zinc-950">
